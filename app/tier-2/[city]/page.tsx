...
export async function generateMetadata({ params }: { params: Promise<{ city: string }> }): Promise<Metadata> {
  ...
  const cityData = tier2Cities[city.toLowerCase()];
  ...
  return {
    ...
    openGraph: {
      ...
      description: cityData.description,
      url: `${siteUrl}/tier-2/${cityData.slug}`,
      images: [{ url: heroImage }],
    }
  };
}

export default async function Tier2CityPage({ params }: { params: Promise<{ city: string }> }) {
  const { city } = await params;
  const cityData = tier2Cities[city.toLowerCase()];
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-white">
        <div className="text-center">
          ...
          <p className="text-gray-500 mb-8">We couldn&apos;t find curated data for this Tier 2 destination.</p>
  ...
  const uniqueDistrictsMap = new Map<string, Destination>();
  ...
  // Fallback to the hardcoded spots if we didn't extract any districts
  const enrichedHotspots = uniqueDistrictsMap.size > 0 
    ? Array.from(uniqueDistrictsMap.values()).map((dest, i) => ({
        ...
        name: districtToEnglish[dest.district] || dest.district,
        description: dest.description || `${districtToEnglish[dest.district] || dest.district} is a stunning representation of ${dest.city}'s unique character.`,
        dynamicImage: dest.imagePath,
        tags: [cityData.name, `Tier ${dest.tier}`]
      }))
    : cityData.hotspots.map((spot: Hotspot): EnrichedHotspot => ({ ...spot, dynamicImage: spot.image }));
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-12 gap-y-24 gap-x-8">
            {enrichedHotspots.map((spot, index) => {
              ...
              return (
                <article key={spot.id} className={`${spanClasses} group cursor-pointer`}>
