...
export function generateStaticParams() {
  return getRouteCityLinks()
    .filter((link) => link.routeSlug === 'route-5')
    .map((link) => ({ city: link.citySlug }));
}

export default async function RouteFiveCityPage({ params }: PageProps) {
  const { city } = await params;
  if (!isCityOnRoute('route-5', city)) notFound();

  return (
    <RoutePageShell activeRouteSlug="route-5" activeCitySlug={city}>
