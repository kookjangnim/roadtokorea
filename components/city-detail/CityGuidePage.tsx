...
export async function generateMetadata({
  ...
  const localCityData = getLocalCityDataBySlug(city);
...
export default async function CityPage({
  ...
  const localCityData = getLocalCityDataBySlug(citySlug);
            ...
            {supportProfile && (
              <section className="mb-12 border-b border-stone-200 pb-10">
                ...
                <div className="mt-8 grid gap-4 xl:grid-cols-3">
                  ...
                  <div id="food" className="rounded-[1.5rem] border border-stone-200 bg-white p-6">
                    <p className="text-[10px] font-semibold uppercase tracking-[0.28em] text-stone-500">
                      Food Logic
                    </p>
                    <p className="mt-4 text-sm leading-7 text-stone-700">{supportProfile.foodSummary}</p>
                  </div>
                  <div id="next-city" className="rounded-[1.5rem] border border-stone-200 bg-white p-6">
                    <p className="text-[10px] font-semibold uppercase tracking-[0.28em] text-stone-500">
                      Next Leg
                    </p>
                ...
                <div id={supportProfile.stayZones?.length ? undefined : 'lodging'} className="mt-8">
                ...
                <div id="attractions" className="mt-8 grid gap-4">
            ...
            {rawContent ? (
              ...
              <div className="space-y-10">
                ...
                {localGallery.length > 0 && (
                  <section
                    id={supportProfile ? undefined : 'attractions'}
            ...
            <div className="grid gap-6 md:grid-cols-2 xl:grid-cols-4">
              {dynamicPosts.map((post: WPPost) => {
                ...
                for (const taxonomyArray of wpTerms) {
                  const category = taxonomyArray.find((term) => term.taxonomy === 'category');
                  if (category) {
